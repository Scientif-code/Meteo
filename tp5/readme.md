...
## Votre travail
Voici les étapes à suivre pour travailler sur le projet dans le dossier `tp5/` :

# Placez-vous dans le dossier `tp5/` du dépôt :
```bash  	  
../tp5$ npm install
```  
#Ensuite
...
../tp5$ npm run dev-server
```  
#Enfin si modification pour mettre à jour
```bash  	  
../tp5$ npm run build
```  
...
Tout est fait. En revanche pour le min et le max du graphique cela n'est pas réellement expliqué donc je l'ai fait à ma manière :  
- min on met 0 si tout positif sinon -5
- max on fait partie entière de max + 5
